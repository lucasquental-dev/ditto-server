...
app.get('/analisar-layout', async (req, res) => {
  try {
    ...
    const geminiData = await geminiComRetry({
      ...
      contents: [{ parts: [{ text: `Você é um consultor RIGOROSO de marketing digital avaliando o site: ${site}\n\n${htmlResumo}\n\nINSTRUÇÃO CRÍTICA: Retorne APENAS o JSON, sem texto antes ou depois, sem markdown.\n\nESCALA OBRIGATÓRIA — seja honesto e rigoroso:\n1-2: Site inacessível, completamente quebrado ou inexistente\n3-4: Template genérico sem identidade, confuso, amador\n5: Funcional mas sem nenhum diferencial visual — isso inclui a MAIORIA dos sites de pequenas empresas\n6: Acima da média local, alguma identidade, mas falhas claras\n7-8: Identidade visual forte, hierarquia clara, profissional de verdade\n9-10: Referência absoluta do setor. EXTREMAMENTE raro.\n\nPENALIZAÇÕES OBRIGATÓRIAS (desconte da nota):\n- Wix, Builderall ou qualquer template genérico: -1 ponto (mencione nas falhas)\n- Sem foto real da equipe: -1 ponto\n- Textos genéricos, sem personalidade ("somos os melhores", "qualidade garantida"): -1 ponto\n- Menu com mais de 6 itens sem hierarquia clara: -0.5 ponto\n- CTA fraco ou ausente: -0.5 ponto\n- Sem SSL: -1 ponto\n\nVALORIZAÇÕES:\n- Foto real da equipe: +1 ponto\n- Identidade visual única e coesa: +1 ponto\n- Textos específicos e personalizados: +0.5 ponto\n\nREGRAS:\n- Ser funcional NÃO é suficiente para nota acima de 5\n- Números zerados (0%, R$0): IGNORE\n- oncorretor.com.br: -1 ponto adicional\n- Cada item das listas: NO MÁXIMO 8 palavras, específico sobre ESTE site\n\nA conclusao deve ser direta e honesta: se o site é bom, diga o que está funcionando bem e o que pode melhorar. Se tem problema sério, aponte de forma construtiva e sem ser deselegante. Nunca force um defeito onde não existe. Linguagem simples, sem jargão técnico. Máximo 60 palavras.\n\nRETORNE APENAS este JSON:\n{\n  "nota": número 1-10,\n  "nota_seo": número 1-10,\n  "transmite_confianca": true ou false,\n  "resumo": "primeira impressão honesta em até 100 caracteres",\n  "analise_nota": "o que viu de concreto: cores, fontes, imagens, layout deste site",\n  "impacto_negocio": ["máx 8 palavras", "máx 8 palavras", "máx 8 palavras"],\n  "principais_falhas": ["máx 8 palavras", "máx 8 palavras", "máx 8 palavras"],\n  "oportunidades": ["máx 8 palavras", "máx 8 palavras", "máx 8 palavras"],\n  "conclusao": "60 palavras máx: maior problema visual deste site, linguagem simples e direta."\n}` }] }]
    });
    const parts = geminiData.candidates?.[0]?.content?.parts || [];
    const textPart = parts.find(p => p.text && !p.thought);
    const text = textPart?.text || '';
